...
impl ShadowRenderer {
    ...
    fn build_silhouette_cache(&mut self, model: &MD3Model, mesh_idx: usize) -> Option<()> {
        ...
        for ((v0, v1), tris) in edge_map.iter() {
            if tris.len() == 1 {
                edges.push(Edge { v0: *v0 as u32, v1: *v1 as u32 });
            }
        }
    ...
    fn build_shadow_volume(
        ...
        for tri in cap_triangles {
            ...
            vertices.push(ShadowVolumeVertex { position: [tri[0].x, tri[0].y, tri[0].z], extrude: 0.0 });
            vertices.push(ShadowVolumeVertex { position: [tri[1].x, tri[1].y, tri[1].z], extrude: 0.0 });
            vertices.push(ShadowVolumeVertex { position: [tri[2].x, tri[2].y, tri[2].z], extrude: 0.0 });
