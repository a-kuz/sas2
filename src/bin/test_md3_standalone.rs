...
use sas2::engine::renderer::{WgpuRenderer, MD3Renderer, WgpuTexture};
use sas2::engine::math::{Orientation, axis_from_mat3, orientation_to_mat4, attach_rotated_entity};
use sas2::engine::loader::{load_textures_for_model_static, load_weapon_textures_static};
...
impl MD3TestApp {
    ...
    fn compute_model_center(model: &MD3Model) -> Vec3 {
        ...
        for mesh in &model.meshes {
            if let Some(frame_verts) = mesh.vertices.get(0) {
                for v in frame_verts {
                    let p = Vec3::new(
                        v.vertex[0] as f32 / 64.0,
                        v.vertex[1] as f32 / 64.0,
                        v.vertex[2] as f32 / 64.0,
                    );
                    min = min.min(p);
                    max = max.max(p);
                    any = true;
    ...
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        if self.window.is_none() {
            ...
            if let Some(path) = lower_path {
                ...
                self.player_center = Self::compute_model_center(&model);
        ...
        match event {
            ...
            WindowEvent::RedrawRequested => {
                ...
                if let (Some(ref mut wgpu_renderer), Some(ref mut md3_renderer)) = 
                    ...
                    let camera_target = self.player_center;
                    let camera_pos = camera_target + Vec3::new(0.0, 0.0, self.camera_distance);
                    ...
                    let lights: Vec<(Vec3, Vec3, f32)> = match self.num_lights {
                        0 => vec![],
                        1 => vec![(light_pos0, light_color0, light_radius0)],
                        2 => vec![(light_pos0, light_color0, light_radius0), (light_pos1, light_color1, light_radius1)],
                        _ => vec![(light_pos0, light_color0, light_radius0), (light_pos1, light_color1, light_radius1)],
                    };
